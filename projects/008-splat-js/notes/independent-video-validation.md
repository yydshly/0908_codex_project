...
实验记录 ID：`run_1788885522715_341819`。原应用将自有捕获的运行名称保存为 Local Scene。指标保存在 [result.json](../experiments/video-sample/result.json)，原始相机与训练数据为 [original-recon.json](../experiments/video-sample/original-recon.json)。
...
- [model.sog](../../../web/008-splat-js/video-assets/model.sog)：原应用压缩后存入 IndexedDB 的模型原样保存。
- [recon.json](../../../web/008-splat-js/video-assets/recon.json)：仅改显示名称、照片 URL 并添加来源说明；相机、点云、训练参数和统计与原记录一致。
...
对照页面：[video-test.html](../../../web/008-splat-js/video-test.html)。包含原视频播放、独立三维查看、实测指标和重新训练入口。
